...
def build_scenarios() -> list[Scenario]:
    return [
        Scenario(
            ...
            SharedEnterprisePromptTrace().generate(sessions=100, shared_prefix_tokens=8192, unique_tokens_per_session=1024, decode_steps=64),
            "100 sessions share a large enterprise policy/system prompt.",
        ),
        Scenario(
            "scenario_02_shared_prefix_1000_sessions",
            SharedEnterprisePromptTrace().generate(sessions=1000, shared_prefix_tokens=4096, unique_tokens_per_session=512, decode_steps=24),
            "1000 sessions share an enterprise prefix; fanout dominates.",
        ),
        Scenario(
            "scenario_03_agentic_tool_loop",
            AgenticLoopTrace().generate(sessions=64, tools_per_session=4, reflection_steps=8, decode_steps=64),
        ...
        Scenario(
            ...
            MultiTenantRackTrace().generate(tenants=8, sessions_per_tenant=32, shared_prefix_probability=0.7, decode_steps=48),
...
def run_scenario(scenario: Scenario) -> list[dict[str, Any]]:
    ...
    for policy in POLICY_MATRIX:
        ...
        row = {
            ...
            "cross_rack_traffic_gb": max(0.0, bytes_to_gb(metrics.bytes_moved - metrics.avoided_cross_rack_bytes)),
...
def run_all_scenarios(output_dir: Path) -> pd.DataFrame:
    ...
    (output_dir / "scenario_findings.md").write_text(ResultInterpreter(frame).to_markdown(), encoding="utf-8")
