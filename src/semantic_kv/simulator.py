...
class SimulationEngine:
    ...
    def _access_block(self, block_id: str, step: int) -> None:
        ...
        if block.tier is not MemoryTier.GPU_HBM:
            ...
            queue_delay = self._queue_delay_us(block.tier)
            stall = max(0, latency - self.tiers[MemoryTier.GPU_HBM].latency_us) + queue_delay
    ...
    def _record_occupancy(self, step: int) -> None:
        ...
        self.metrics.latency_history.append(
            {"step": step, "stall_us": self.metrics.simulated_stall_us}
        )
    ...
    def _record_transfer(
        ...
        penalty = transfer_us * hop_penalty + queue_depth * self.tiers[target].latency_us * 0.02
    ...
    def _throughput_score(self) -> float:
        stall_penalty = self.metrics.simulated_stall_us / 1_000_000
        ...
        return max(0.01, 1.0 + reuse_bonus - stall_penalty - movement_penalty - saturation_penalty)
