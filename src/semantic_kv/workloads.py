...
class EventType(StrEnum):
    """Event kinds emitted by synthetic workload generators."""
...
class WorkloadEvent:
    """Represent a simulator event generated by a workload."""
...
def basic_decode_workload(
    ...
    """Generate a baseline decode workload with active tail blocks."""
    ...
    for s in range(sessions):
        ...
        for i in range(blocks_per_session):
            klass = (
                ...
                if i >= blocks_per_session - 2
                else EvictionClass.SESSION_RECENT
            )
            events.append(
                WorkloadEvent(i, EventType.CREATE_BLOCK, _block(profile, session_id, i, i, klass))
            )
    for step in range(blocks_per_session, blocks_per_session + decode_steps):
        for s in range(sessions):
            events.append(
                WorkloadEvent(
                    step, EventType.ACCESS_BLOCK, block_id=f"s{s}:b{blocks_per_session - 1}"
                )
            )
    return events
...
def shared_prefix_workload(
    ...
    """Generate sessions that share a large exact-match prefix."""
        ...
        for i in range(blocks_per_session):
            ...
            events.append(
                WorkloadEvent(
                    ...
                    _block(profile, session_id, i, i, klass, phash, fanout_count=fanout),
                )
            )
    for step in range(blocks_per_session, blocks_per_session + decode_steps):
        for s in range(sessions):
...
def agentic_tool_workload(
    ...
    """Add ephemeral tool-call KV on top of the shared-prefix workload."""
        ...
        for i in range(3):
            events.append(
                WorkloadEvent(
                    ...
                    EventType.CREATE_BLOCK,
                    _block(
                        profile, session_id, base + i, base + i, EvictionClass.EPHEMERAL_TOOL_CALL
                    ),
                )
            )
...
class AgenticWorkflowWorkload:
    ...
    def generate(
        self, profile: ModelProfile, sessions: int, context: int, decode_steps: int
    ) -> list[WorkloadEvent]:
                ...
                events.append(
                    WorkloadEvent(
                        ...
                        _block(
                            ...
                            prefix,
                            tenant_id,
                            fanout_count=sessions // 8,
                        ),
                    )
                )
        return sorted(
            events + basic_decode_workload(profile, sessions, profile.block_tokens, decode_steps),
            key=lambda e: e.step,
        )
...
def long_context_workload(
    ...
    """Generate a few very large-context sessions."""
...
def mixed_tenant_workload(
    ...
    """Generate multiple tenants with a mix of cold and recent state."""
...
def agentic_workflow_workload(
    profile: ModelProfile, sessions: int, context: int, decode_steps: int
) -> list[WorkloadEvent]:
    """Generate an agentic workflow workload via the workload class wrapper."""

    return AgenticWorkflowWorkload().generate(profile, sessions, context, decode_steps)


def multi_tenant_inference_workload(
    profile: ModelProfile, sessions: int, context: int, decode_steps: int
) -> list[WorkloadEvent]:
    """Generate a multi-tenant inference workload via the workload wrapper."""

    return MultiTenantInferenceWorkload().generate(profile, sessions, context, decode_steps)


def shared_enterprise_prompt_workload(
    profile: ModelProfile, sessions: int, context: int, decode_steps: int
) -> list[WorkloadEvent]:
    """Generate a large shared enterprise prompt workload."""

    return SharedEnterprisePromptWorkload().generate(profile, sessions, context, decode_steps)


def multi_agent_collaboration_workload(
    profile: ModelProfile, sessions: int, context: int, decode_steps: int
) -> list[WorkloadEvent]:
    """Generate collaborative agents with partial shared memory."""
...
def make_workload(
    ...
    """Construct a named workload and return events sorted by step."""
    ...
    return sorted(
        generators[normalized](profile, sessions, context, decode_steps), key=lambda e: e.step
    )
